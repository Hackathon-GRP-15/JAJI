...
4. Copy .env.example
6. Check variable in .env : `DATABASE_URL="postgresql://symfony:ChangeMe@database:5432/app?serverVersion=13&charset=utf8"`
7. Run `docker compose exec php bin/console d:f:l -n` to run fixtures
8. Run ` docker compose exec php npm install` to install dependencies.
9. Run ` docker compose exec php npm run build` to compile.
10. Open `https://localhost` in your favorite web browser and [accept the auto-generated TLS certificate](https://stackoverflow.com/a/15076602/1352334)
11. Run `docker compose down --remove-orphans` to stop the Docker containers.
12. Run `docker compose logs -f` to display current logs, `docker compose logs -f [CONTAINER_NAME]` to display specific container's current logs
